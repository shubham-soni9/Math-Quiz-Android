...
class CommonLogic {
    ...
    private static void applyGravityToLine(LineDefinition line, ConfigDefinition config) {
    ...
    private static int getGravityFromRelative(int childGravity, ConfigDefinition config) {
    ...
    public static void fillLines(List<ViewDefinition> views, List<LineDefinition> lines, ConfigDefinition config) {
        ...
        for (int i = 0; i < count; i++) {
            ...
            if (newLine && config.getMaxLines() > 0 && lines.size() == config.getMaxLines()) {
                //currentLine.removeLastView(views.get(i-1));
